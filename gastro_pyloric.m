...
%%
% Now I plot PD burst metrics triggered by when LG starts and ends. 


% look at how PD ISIs (IBIs) change triggered around LG start/end
...
for exp_idx = 1:length(data)
	...
	isis = diff(data(exp_idx).PD);
	x = data(exp_idx).PD(1:end-1);
...
xlabel(ax(7),'Time since LG start')
